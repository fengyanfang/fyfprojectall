...
frameApp.controller('lessonCtrl', function ($scope,$http,shareSvc) {
    ...
    $scope.pageChangeTo = function(pageNumber) {
        ...
        var url = '/api/teacher/sq_teacher/?action=list_lesson&pagenum='+pageNumber  + '&pagesize='+$scope.pageSize;
    ...
    // 得到课程列表
    Util.angular_get($http,
        '/api/teacher/sq_teacher/?action=get_teach_courses',
        function (ret) {
    ...
    $scope.showCheckins = function(lesson) {
        ...
        Util.angular_get($http,
            '/api/teacher/sq_teacher/?action=list_lesson_checkins&lessonid='+lesson.id,
            function (ret) {
