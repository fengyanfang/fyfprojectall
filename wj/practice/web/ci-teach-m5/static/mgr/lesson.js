...
frameApp.controller('lessonCtrl', function ($scope,$http,shareSvc) {
    ...
    $scope.pageChangeTo = function(pageNumber) {
        ...
        var url = '/api/mgr_lesson?action=list_lesson&pagenum='+pageNumber  + '&pagesize='+$scope.pageSize;
    ...
    // 得到课程列表
    Util.angular_get($http,
        '/api/mgr_course?action=list_course&pagenum=1&pagesize=100',
        function (ret) {

            $scope.courseList = ret.data;
        }
    );
    ...
    $scope.showCheckins = function(lesson) {
        ...
        Util.angular_get($http,
            '/api/mgr_lesson?action=list_lesson_checkins&lessonid='+lesson.id,
            function (ret) {
